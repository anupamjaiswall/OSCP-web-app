...
test('browser render is a required bounded CI gate',()=>{const ci=read('.github/workflows/ci.yml'),smoke=read('scripts/browser-smoke.mjs');ok(ci.includes('Headless browser render gate'));ok(!ci.includes('continue-on-error: true'));ok(smoke.includes("'--timeout=7000'"));ok(smoke.includes('--screenshot='));ok(smoke.includes('[1,1.25,1.5]'));ok(smoke.includes('globalSearch'));ok(smoke.includes('serviceRouterView'));ok(smoke.includes('buf.length>=20000'));ok(smoke.includes("child.kill('SIGKILL')"));ok(smoke.includes('Date.now()+18000'));ok(!smoke.includes('virtual-time-budget'));ok(!smoke.includes('run-all-compositor-stages-before-draw'));ok(!smoke.includes('remote-debugging-port'));ok(!smoke.includes('remote-debugging-pipe'));ok(!smoke.includes("'--dump-dom'"))});
