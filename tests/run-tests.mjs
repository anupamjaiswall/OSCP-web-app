...
test('browser smoke is a required CI gate',()=>{const ci=read('.github/workflows/ci.yml'),smoke=read('scripts/browser-smoke.mjs');ok(ci.includes('Headless browser boot and interaction gate'));ok(!ci.includes('continue-on-error: true'));ok(smoke.includes("'--dump-dom'"));ok(smoke.includes("'--virtual-time-budget=5000'"));ok(smoke.includes("boot('125% scale','1.25')"));ok(smoke.includes("boot('150% scale','1.5')"));ok(smoke.includes('id="readerContrastToggle"'));ok(smoke.includes('id="refNavigator"'));ok(!smoke.includes('remote-debugging-port'))});
