...
test('browser render is a required bounded CI gate',()=>{const ci=read('.github/workflows/ci.yml'),smoke=read('scripts/browser-smoke.mjs');ok(ci.includes('Headless browser render gate'));ok(!ci.includes('continue-on-error: true'));ok(smoke.includes('remote-debugging-port=9222'));ok(smoke.includes("'about:blank'"));ok(smoke.includes("Page.navigate',{url:appUrl}"));ok(smoke.includes("http.createServer"));ok(smoke.includes("server.listen(0,'127.0.0.1'"));ok(smoke.includes('Runtime.evaluate'));ok(smoke.includes('Page.captureScreenshot'));ok(smoke.includes('Emulation.setDeviceMetricsOverride'));ok(smoke.includes('[1,1.25,1.5]'));ok(smoke.includes('globalSearch'));ok(smoke.includes('serviceRouterView'));ok(smoke.includes('Runtime.exceptionThrown'));ok(smoke.includes("child.kill('SIGKILL')"));ok(!smoke.includes("'--dump-dom'"));});
