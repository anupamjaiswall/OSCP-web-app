...
test('deep reference is inert during parser boot and hydrated after load',()=>{const tpl=read('src/index.template.html'),build=read('scripts/build.mjs'),h=read('src/js/20-reference-hydrator.js'),core=read('src/js/03-core-app.js');ok(tpl.includes('id="referencePayload"'));ok(tpl.includes('@inject:reference-payload'));ok(!tpl.includes('@inject:reference -->'));ok(build.includes('referencePayload=JSON.stringify(referenceHtml)'));ok(build.includes("replace('<!-- @inject:reference-payload -->',()=>referencePayload)"));ok(h.includes("window.addEventListener('load'"));ok(h.includes('createContextualFragment'));ok(h.includes("setTimeout(schedule,0)"));ok(h.includes("CustomEvent('oscp-reference-ready')"));ok(core.includes('if(!window.OSCP_REFERENCE_READY)'))});
