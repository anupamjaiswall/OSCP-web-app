...
(()=>{
 ...
 function searchItem(anchor){try{const items=typeof ensureSearchItems==='function'?ensureSearchItems():(typeof SEARCH_ITEMS!=='undefined'?SEARCH_ITEMS:[]);return Array.isArray(items)&&items.length?items.find(x=>x.anchor===anchor):null}catch(_){return null}}
