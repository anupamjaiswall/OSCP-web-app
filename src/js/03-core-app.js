...
function renderFavorites(){const root=$('#favoriteList');if(!root)return;const items=favorites.map(a=>{const indexed=SEARCH_ITEMS.find(x=>x.anchor===a);if(indexed)return indexed;const el=document.getElementById(a);if(!el)return null;return{anchor:a,title:(el.querySelector?.('summary')?.textContent||el.textContent||a).replace(/\s+/g,' ').trim().slice(0,120),tags:[]}}).filter(Boolean);root.innerHTML=items.length?items.map(x=>`<div class="favorite"><div><b>${esc(x.title)}</b><div class="tiny">${esc((x.tags||[]).slice(0,4).join(' '))}</div></div><div><button class="btn favOpen" data-a="${esc(x.anchor)}">Open</button><button class="btn bad favRemove" data-a="${esc(x.anchor)}">×</button></div></div>`).join(''):'<div class="muted">Pin useful search results and they stay here.</div>';$$('.favOpen').forEach(b=>b.onclick=()=>openRef(b.dataset.a));$$('.favRemove').forEach(b=>b.onclick=()=>toggleFavorite(b.dataset.a))}
function renderSearch(q){q=q||'';let scored=ensureSearchItems().map(x=>({...x,score:fuzzyScore(x,q)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).slice(0,60);$('#searchStats').textContent=q?`${scored.length} best matches for “${q}”`:'Type a tag, port, error, privilege, tool, or clue.';$('#searchResults').innerHTML=scored.map(x=>`<div class="result"><div class="row" style="justify-content:space-between"><div class="resultTitle">${esc(x.title)}</div><div><span class="score">score ${x.score}</span> <button class="btn pinRef" data-pin="${esc(x.anchor)}">${favorites.includes(x.anchor)?'★':'☆'} Pin</button></div></div><div class="chips">${(x.tags||[]).slice(0,8).map(t=>`<span class="chip click" data-tag="${esc(t)}">${esc(t)}</span>`).join('')}</div><div class="resultText">${esc(x.text.replace(/\s+/g,' ').slice(0,520))}</div><button class="btn" data-open="${esc(x.anchor)}">Open in full reference →</button></div>`).join('')||'<div class="card muted">No match. Try a shorter term or exact [TAG].</div>';$$('[data-open]').forEach(b=>b.onclick=()=>openRef(b.dataset.open));$$('[data-tag]').forEach(c=>c.onclick=()=>{$('#globalSearch').value=c.dataset.tag;renderSearch(c.dataset.tag)});$$('[data-pin]').forEach(b=>b.onclick=()=>toggleFavorite(b.dataset.pin))}
...
function renderExternalBackupFreshness(){
 ...
 const at=Number(externalBackupMeta?.at)||0,kind=String(externalBackupMeta?.kind||'backup');
 if(!at){el.textContent='External backup: none recorded in this browser profile yet.';el.dataset.state='warn';return}
 const age=Math.max(0,Date.now()-at),minutes=Math.floor(age/60000),when=new Date(at).toLocaleString();
 el.textContent='Last external '+kind+' backup: '+when+(minutes<1?' · just now':minutes<60?' · '+minutes+'m ago':' · '+Math.floor(minutes/60)+'h ago');
...
$('#exportEncrypted').onclick=async()=>{
 ...
 try{const obj=await encryptPayload(sessionPayload($('#encIncludeSecrets').checked),pass);downloadText('oscp-encrypted-backup.json',JSON.stringify(obj,null,2),'application/json');markExternalBackup('encrypted session');toast('Encrypted backup exported')}catch(e){alert('Encryption failed: '+e.message)}
};
