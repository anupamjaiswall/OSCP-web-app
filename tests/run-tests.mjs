...
test('browser smoke is a required CI gate',()=>{const ci=read('.github/workflows/ci.yml'),smoke=read('scripts/browser-smoke.mjs');ok(ci.includes('Headless browser boot and interaction gate'));ok(!ci.includes('continue-on-error: true'));ok(smoke.includes("'--remote-debugging-pipe'"));ok(smoke.includes('Target.createTarget'));ok(smoke.includes('Target.attachToTarget'));ok(smoke.includes('checkZoom(125)'));ok(smoke.includes('checkZoom(150)'));ok(smoke.includes('uncaught browser exception(s)'));ok(!smoke.includes('remote-debugging-port'));ok(!smoke.includes("'--dump-dom'"))});
