...
test('browser smoke is a required CI gate',()=>{const ci=read('.github/workflows/ci.yml'),smoke=read('scripts/browser-smoke.mjs');ok(ci.includes('Headless browser boot and interaction gate'));ok(!ci.includes('continue-on-error: true'));ok(smoke.includes('/json/new?'));ok(!smoke.includes('Page.loadEventFired'));ok(smoke.includes("4000,'state evaluation'"));ok(smoke.includes('checkZoom(125)'));ok(smoke.includes('checkZoom(150)'));ok(smoke.includes('uncaught browser exception(s)'))});
